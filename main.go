	...
	app.Commands = []*cli.Command{
		{
			...
			Action: func(c *cli.Context) error {
				domain, e := NewDomain(c.String("domain"))
				if e != nil {
		...
		},
		{
			...
			Action: func(c *cli.Context) error {
				domainType, e := NewDomain(c.String("domain"))
				if e != nil {
					return cli.Exit(e, 128)
				}
		...
		},
		{
			...
			Flags: []cli.Flag{
				&cli.StringFlag{Name: "listen", Aliases: []string{"l"}, EnvVars: []string{"ASM_WS_LISTEN"}, Value: "0.0.0.0:9000", DefaultText: "0.0.0.0:9000", Usage: "HTTP address/port want to listen"},
